...
// end include: preamble.js
  ...
  function _CDAdTikTok_HasRewardVideo() {
          if (typeof TTMinis === 'undefined') { return 0; }
          return (window._cdTikTokAdReady === true) ? 1 : 0;
      }

  function _CDAdTikTok_Init() {
          if (typeof TTMinis === 'undefined') { return; }
          window._cdTikTokAdReady = true;
      }

  function _CDAdTikTok_ShowRewardVideo() {
          if (typeof TTMinis === 'undefined') {
              try {
                  unityInstance.SendMessage('AdsHandler', '_OnRewardVideoEventFromJs', 'Failed|TikTok|0');
              } catch(e) {}
              return;
          }
          try {
              var adUnitId = window.CD_TIKTOK_AD_UNIT_ID || '';
              var ad = TTMinis.game.createRewardedVideoAd({ adUnitId: adUnitId });
              ...
              ad.onClose(function (res) {
                  window._cdTikTokAdReady = true;
                  var finished = res && res.isEnded;
                  var evt = finished ? 'Complete' : 'Failed';
                  try {
                      unityInstance.SendMessage('AdsHandler', '_OnRewardVideoEventFromJs', evt + '|TikTok|0');
                  } catch(e) {}
              });
  
              window._cdTikTokAdReady = false;
              try {
                  unityInstance.SendMessage('AdsHandler', '_OnRewardVideoEventFromJs', 'ShowAd|TikTok|0');
              } catch(e) {}
  ...
  function _CDNative_GetOrCreateDeviceId() {
          try {
              var key = 'cd_device_id';
              ...
              var size = lengthBytesUTF8(id) + 1;
              var buf = _malloc(size);
              stringToUTF8(id, buf, size);
              return buf;
          } catch (e) {
              var fallback = '';
              var size2 = lengthBytesUTF8(fallback) + 1;
              var buf2 = _malloc(size2);
              stringToUTF8(fallback, buf2, size2);
              return buf2;
          }
      }

  function _CDNative_TikTok_GetLocale() {
          var locale = '';
          try {
              if (typeof TTMinis !== 'undefined' && TTMinis.game && TTMinis.game.getSystemInfoSync) {
                  var info = TTMinis.game.getSystemInfoSync();
                  locale = (info && info.language) ? info.language : '';
              }
          } catch (e) { locale = ''; }
          ...
          var buf = _malloc(size);
          stringToUTF8(locale, buf, size);
          return buf;
      }

  function _CDNative_TikTok_VibrateLong() {
          if (typeof TTMinis !== 'undefined' && TTMinis.game && TTMinis.game.vibrateLong) {
              try { TTMinis.game.vibrateLong({}); } catch (e) {}
          }
      }

  function _CDNative_TikTok_VibrateShort() {
          if (typeof TTMinis !== 'undefined' && TTMinis.game && TTMinis.game.vibrateShort) {
              try { TTMinis.game.vibrateShort({}); } catch (e) {}
          }
      }

  function _CDSave_Sync() {
          try {
              if (typeof TTMinis !== 'undefined' && TTMinis.game && TTMinis.game.setStorage) {
                  // TikTok: read all .sdt files from virtual FS and persist via platform storage
                  var savePath = '/idbfs' ;
                  var files = {};
                  var hasFiles = false;
                  try {
                      var entries = FS.readdir(savePath + '/cdg/sdts');
                      for (var i = 0; i < entries.length; i++) {
                          var name = entries[i];
                          if (name === '.' || name === '..') continue;
                          if (name.indexOf('.sdt') === name.length - 4) {
                              var content = FS.readFile(savePath + '/cdg/sdts/' + name, { encoding: 'utf8' });
                              files[name] = content;
                              hasFiles = true;
                          }
                      }
                  ...
                  // Fallback: standard IndexedDB sync
                  FS.syncfs(false, function (err) {
                      if (err) {
                          console.warn('[CDSave_Sync] syncfs error:', err);
                      }
                  });
              }
          } catch (e) {
              console.warn('[CDSave_Sync] error:', e);
          }
      }

  function _CDSave_TikTokRestore() {
          if (typeof TTMinis === 'undefined' || !TTMinis.game || !TTMinis.game.getStorage) {
              try {
                  unityInstance.SendMessage('SaveFramework', '_OnTikTokCloudLoaded', '');
              } catch (e) {}
              return;
          }
          try {
              TTMinis.game.getStorage({
                  ...
                  success: function (res) {
                      var json = (res && res.data) ? res.data : '';
                      try {
                          if (json) {
                              var files = JSON.parse(json);
                              var savePath = '/idbfs/cdg/sdts';
                              try { FS.mkdirTree(savePath); } catch (e) {}
                              for (var name in files) {
                                  if (files.hasOwnProperty(name)) {
                                      FS.writeFile(savePath + '/' + name, files[name]);
                                  }
                              }
                          }
                      } catch (e) {
                          console.warn('[CDSave_TikTokRestore] parse error:', e);
                      }
                      try {
                          unityInstance.SendMessage('SaveFramework', '_OnTikTokCloudLoaded', json ? 'ok' : '');
                      } catch (e) {}
                  },
                  fail: function () {
                      try {
...
var wasmImports = {
  "CDAdTikTok_HasRewardVideo": _CDAdTikTok_HasRewardVideo,
  "CDAdTikTok_Init": _CDAdTikTok_Init,
  "CDAdTikTok_ShowRewardVideo": _CDAdTikTok_ShowRewardVideo,
  "CDNative_GetOrCreateDeviceId": _CDNative_GetOrCreateDeviceId,
  "CDNative_TikTok_GetLocale": _CDNative_TikTok_GetLocale,
  "CDNative_TikTok_VibrateLong": _CDNative_TikTok_VibrateLong,
  "CDNative_TikTok_VibrateShort": _CDNative_TikTok_VibrateShort,
  "CDSave_Sync": _CDSave_Sync,
  "CDSave_TikTokRestore": _CDSave_TikTokRestore,
